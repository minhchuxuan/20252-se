...
Energy Optimizer (SHEO) project. It grounds that description in an established framework: the
software development life cycle (SDLC), the classical process models, the agile philosophy,
and the ISO/IEC~9126 quality model. Each section moves from the general concept to its
...
verifiable outputs. Two complementary views of the SDLC are useful here.
...
Six classical process models are commonly distinguished. Table~\ref{tab:process-models} summarises
...
to the Incremental model, augmented by agile working practices described in
Section~\ref{sec:agile}. The selection was justified by the following conditions.
...
\begin{enumerate}[leftmargin=*, label=\textbf{Increment \arabic{enumi}.}]
  ...
  \item \textbf{UI and integration.} React frontend pages, the capability-driven device
        control widget, the user flow, and final integration verification across all 66
        test cases.
\end{enumerate}
...
Agile is a \textit{philosophy} (the 2001 Manifesto~\cite{agile}), distinct from Scrum as a specific
\textit{framework}. SHEO embodied several Agile values and principles but did not run formal
Scrum ceremonies. Each practised value is documented below.
...
increment was required to leave the full test suite passing before integration. The 66
...
original role model was renamed to align with the project's stakeholder taxonomy;
...
ISO/IEC~9126~\cite{iso9126} is the relevant product-quality standard, and quality assurance
must run \textit{throughout} the development process, not only at the end.
Table~\ref{tab:iso9126} maps each ISO/IEC~9126 quality characteristic to the
...
\textbf{Functionality} &
  ...
  recommendations, savings); validated by 66 automated acceptance and integration tests;
...
\textbf{Efficiency} &
  ...
  NFR-PER: API response-time target of $\le 2$\,s at the 95th percentile (per NFR-PER-2;
  a design target, not yet load-tested);
...
what the Agile philosophy prescribes, and it is what enabled SHEO to
reach a state of 66 passing tests (out of 69 --- the other 3 fail, marking genuine
open defects the team has found but not yet fixed) and a fully seeded, demonstrable
product at the point of submission.
