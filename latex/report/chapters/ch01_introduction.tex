...
introduced in Lecture C01 (\emph{Overview of Software Engineering}): software
...
desired quality}. The same lecture frames software engineering as a layered activity:
...
onboards units to residents, and sets the building-wide tariff and billing cycle. The
...
testable entirely in software. Second, the ``AI'' is intentionally \emph{not} a
black-box machine-learning model. It is an \textbf{explainable, rule-based} engine: every
learned habit surfaces as a rule the user can read and every saving figure reduces to a
documented formula. Opaque models were excluded precisely because they would defeat the
client's high-priority demand that the saving be \emph{visible and trustworthy}.
...
timeline. Likewise, a learned black-box model would raise development cost and risk while
\emph{reducing} the function the client values most---the explainable VND saving. In both
cases the chosen scope maximises the function that matters while respecting the cost and
time available, which is exactly the trade-off C01 asks an engineer to make explicit.
