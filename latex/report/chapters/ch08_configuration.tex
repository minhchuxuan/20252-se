...
C5 & Test suite
   & The shared test fixtures and the automated test modules --- sixty-seven functional cases plus three open-defect cases (known failures)
   & QA lead \\[4pt]
...
M1 --- Requirements
  ...
  & Requirements specification approved; all requirement identifiers stable \\[6pt]
M2 --- Design \& skeleton
  & M2
...
M3 --- Implementation
  ...
  & All sixty-seven functional tests pass (three open-defect tests remain failing, marking known bugs); the demonstration seeds twenty-one days of data and the application runs end-to-end \\[6pt]
...
A trunk-plus-feature model is adopted, suited to a small, collocated team
...
\begin{description}[leftmargin=1.6cm, style=nextline]
  \item[Integration line]
    ...
    the full suite of sixty-seven functional tests (the three open-defect tests are
    known failures, tracked outside the gate). Changes are never applied to it
...
must pass through a three-stage change-control process. For a small team this
...
The sixty-seven functional tests must pass on the branch before
merge (the three open-defect tests are known, tracked failures and do not gate the
