...
structure as a software design description in the sense of IEEE~1016~\cite{ieee1016}. Where the
...
intellectual heart of the product (\S\ref{sec:heart}). Throughout, the design is
presented in the conceptual vocabulary of the course, transforming the analysis
model into a design model in the sense of ISO/IEC/IEEE~12207.

\section{Design goals}
\label{sec:design-goals}

The design is governed by the four fundamental concepts taught in C07: abstraction,
...
\begin{table}[htbp]
\centering
\caption{Each C07 design concept and the SHEO mechanism that realises it.}
...
composes three further architectural styles taught in C07, each selected to satisfy
...
in the spirit of the C07 candidate-assessment step.
...
\begin{itemize}[leftmargin=1.4em]
  ...
  would force an external infrastructure dependency that a single-node course demo
...
\textbf{entity/boundary/control} separation taught in C07. \emph{Entity} classes
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{Recommendation service} --- mines daily habits from telemetry into
  human-readable \textsc{when}--\textsc{then} suggestions with a rationale and a VND
  estimate, ranks and caps them, turns an accepted suggestion into a rule, and
  suppresses a dismissed one by signature.
...
Five design patterns give the structure its flexibility; each is realised by a
...
type adds a strategy without editing callers --- the open/closed principle (OCP). \\
Observer (GoF) &
...
The rule and execution lifecycles are modelled as explicit states (for example a rule
moving through draft, enabled, and disabled; an execution through applied, undone, and
expired), so behaviour follows the lifecycle rather than scattered conditionals. \\
\bottomrule
\end{tabularx}
\end{table}
...
which the design constraint requires to be transparent rather than a black box. Every
