...
Software construction is the activity of converting a
...
Web framework & A declarative Python web framework~\cite{fastapi} &
...
Persistence & A mature object-relational mapper~\cite{sqlalchemy} &
...
\midrule
User-interface framework & A component-based front-end framework~\cite{react} &
...
Six coding-style principles guide the codebase: \emph{consistency},
...
The persistence layer is organised into repositories---one per aggregate (for example,
one for devices and one for the per-device permission records)---that encapsulate all
query construction. Service code
...
Refactoring~\cite{fowler} is the transformation of the internal structure of
...
extend---prior to the addition of new capability. Common code smells
include duplicated code, long method, large class, too many parameters,
...
Both refactorings satisfy the defining criterion of refactoring: no behaviour changed---the automated test
...
Two review processes are commonly distinguished. A \textbf{walkthrough} is author-led
...
checklist; the author is present but does not lead. Three review
criteria are applied: \emph{maintainability} (can the code be changed safely?),
...
Table~\ref{tab:review-criteria} maps each review criterion to the mechanism
that satisfies it after the inspection.

\begin{table}[htbp]
\centering
\caption{Code review criteria and the corresponding SHEO mechanisms
post-inspection.}
...
tests the code against the stated contract. This is precisely the rationale for
requiring formal inspections rather than relying on walkthroughs alone.
