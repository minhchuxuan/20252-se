...
(SHEO) must do, deliberately kept separate from \emph{how} it is built. Following
the discipline taught in Lecture C06 (Requirement Analysis), it first walks through
...
\section{The Requirements Engineering Process (C06)}
\label{sec:req-process}

Lecture C06 frames requirements engineering as a five-step pipeline whose output is a
Software Requirements Specification (SRS). SHEO followed this pipeline faithfully:

\begin{enumerate}
  \item \textbf{Feasibility study.} Before committing, the team checked that the
  product was technically, operationally, and economically achievable within a single
  course timeline. The decisive constraint --- no physical Internet-of-Things (IoT)
  ...
  resident would inspect consumption, control a device, and act on a suggestion). The
  course's own client-discussion practice --- in which each group posed sharp scoping
  questions to the instructor acting as client --- supplied the model for this step.
  ...
  \item \textbf{Modelling.} The analysed requirements were captured in the standard
  C06 models: a use-case diagram and use-case specification tables for the functional
  ...
  and test. C06's quality criteria for an SRS --- that it be \emph{clear, accurate,
...
A central rule from C01 and C06 underpins the entire chapter: requirements
...
\textbf{Who the client is.} Per the C01 stakeholder taxonomy, the client is the party
...
--- are distinguished from the client throughout, exactly as C01 requires.
...
\textbf{Relation to the course's client-Q\&A norm.} The course runs a recurring
practice in which each group, having received a terse client brief, must return to the
client with sharp questions that pin down scope, roles, and edge cases before
designing anything --- for instance, questions on who may publish a public ticket, how
delivery is handled, or whether an offline mode is required. SHEO's elicitation
mirrors this norm. The team's clarifying questions resolved exactly the points the
...
Following C06, non-functional requirements are classified as \emph{product},
...
\begin{table}[htbp]
\centering
\caption{Non-functional requirements, classified per C06 (product / external).}
\label{tab:nfr}
\small
...
\midrule
\req{BR-1} & Only the Administrator (building owner) may onboard residents by selling them a unit, which is provisioned with its default device package; a Resident cannot add or remove devices. & H\\
...
system as a single black box used by a small set of external actors. In keeping with
the C06 convention, each actor is named with a \emph{singular role noun} (not an
individual or a job title), and each use case is named \emph{verb + noun}.

\fig{fig04-use-case.png}{Use-case model. The deployment models one apartment building. The Administrator (building owner) monitors a building overview across every unit, configures the building-wide tariff, and onboards residents by selling them a unit --- but never manually operates a unit's devices and authors no automation rules; the Resident operates only their own unit's devices and may author rules and accept recommendations for that unit; the Developer/Tester exercises mock devices on a maintenance unit; the System Scheduler autonomously fires rules and mines habits. The Client (IoT contractor) and Customer (building management) are business stakeholders outside the boundary --- the Customer operates the system \emph{as} the Administrator. Accepting a recommendation $\langle\langle$include$\rangle\rangle$ authoring a rule, which in turn $\langle\langle$include$\rangle\rangle$ command validation and a VND estimate.}{fig:usecase}
...
\begin{itemize}
  ...
  configures the building-wide tariff, and onboards residents by selling them a unit
  (provisioned with its default device package).
...
command-validation and VND-estimation steps. As C01 requires, the Client and Customer
...
\section{Use-Case Specifications (C06)}
\label{sec:uc-spec}

C06 requires that the principal use cases be detailed in specification tables giving
