...
This chapter documents the project management artifacts for SHEO following the
framework taught in Lecture~C04: the four~Ps, stakeholder analysis, Work Breakdown
Structure (WBS), effort estimation, scheduling, and risk management. These artifacts
are constructed to satisfy the course deliverable requirements and to reflect the
actual scope and constraints of a small student team building a software-only system.
...
\subsection{People}
The development team consists of four undergraduate students, each taking on
...
Python), frontend development (React/TypeScript), systems integration, and
documentation.
...
Each increment closed with automated regression testing (61 test cases) and a
peer code-review pass.  This mirrors the Incremental model described in~C02 and
aligns with Agile values of working software and customer collaboration.

\subsection{Project}
The project is a temporary, unique academic endeavour with a fixed delivery date.
Key constraints are: no physical IoT hardware available (mitigated by a
deterministic simulator), a four-person team with limited availability, and a
requirement to satisfy course deliverables in parallel with development.  Success
...
Table~\ref{tab:pm-stakeholders} identifies all stakeholders, their role in the C01
framework, and their principal interests and influence over the project.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{>{\bfseries}L{2.5cm} L{2.3cm} Y Y}
    ...
    Development Team
      & Developer / Tester
      & Deliver a correct, maintainable product within course constraints
...
\begin{longtable}{@{}p{0.6cm} p{3.8cm} p{7.6cm}@{}}
  ...
  9.1 & Unit and integration test suite
      & Automated suite covers all REQ-4.x items; all 61 test cases pass on a clean checkout. \\[2pt]
  ...
  10.3 & Project management report
      & This chapter is produced and included in the course deliverable package. \\
\end{longtable}
...
L~=~7~person-days), which is consistent with the effort-table approach taught
in~C04.  Table~\ref{tab:estimation} summarises the result.  Total
estimated effort is approximately \textbf{68 person-days} across the four-member
team over a twelve-week period (approximately 17~person-days per team member).
...
by the Critical Path Method.  All durations assume a parallel team of four with
partial time availability (estimated effective capacity of approximately 40\% per
day per member during semester).
...
Risk management followed the five-step process from~C04: plan, identify, analyse
...
\begin{longtable}{@{} p{2.5cm} p{1.4cm} p{0.7cm} p{1.4cm} p{0.7cm} p{1.5cm} p{4.3cm} @{}}
  ...
  Cross-unit data leakage: a Resident accesses another unit's data via the API (IDOR)
    ...
    & 3
    & Mitigate
    & All database queries are scoped by the unit identifier carried in the verified session token; an adversarial code-review pass specifically targeted cross-unit access vulnerabilities; all findings were corrected and re-tested before delivery. \\[4pt]
