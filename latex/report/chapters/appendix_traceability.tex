...
linking each requirement to the design element that realises it and the automated test
that verifies it. It supports both forward (requirement~$\rightarrow$~component~$\rightarrow$~test)
...
drift) are centralised in the system configuration module. The full suite is \textbf{61 automated tests,
all passing}.
...
BR-1 & Only the Administrator sells units / onboards residents & The administrator-authorisation guard and the resident-onboarding endpoint of the authentication API & Verifies that a resident-role user cannot add a device, and that the Administrator can sell a unit --- provisioned with its default device package --- to a new Resident who then controls that unit but cannot reach another \\
...
\midrule
Strategy & Per-device-type behaviour and power model encapsulated in interchangeable adapters & The device-adapter interface and its per-type implementations \\
